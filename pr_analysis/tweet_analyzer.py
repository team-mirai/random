...
def parse_arguments():
    """コマンドライン引数をパースする"""
    parser = argparse.ArgumentParser(description="X.com（旧Twitter）からハッシュタグを含むツイートを取得し、GitHub URLを分析するツール")
    
    parser.add_argument("--mode", choices=["fetch", "report", "both"], default="both",
                       help="実行モード: fetch=ツイート取得のみ, report=レポート生成のみ, both=両方実行 (デフォルト: both)")
    
    parser.add_argument("--limit", type=int, default=100,
                       help="取得するツイートの最大数 (デフォルト: 100)")
    parser.add_argument("--since", type=str,
                       help="この日付以降のツイートを取得 (形式: YYYY-MM-DD)")
    parser.add_argument("--until", type=str,
                       help="この日付以前のツイートを取得 (形式: YYYY-MM-DD)")
    
    parser.add_argument("--json-file", type=str,
                       help="レポート生成に使用するJSONファイルのパス (mode=reportの場合に必須)")
    ...
    if args.mode in ["fetch", "both"]:
        ...
        if args.mode == "both":
            generate_reports(args, 
                           ...
                           tweets_data=result["tweets_data"])
